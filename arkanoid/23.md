...
既然会有多个球，就需要一个类似数组的容器来存放它们，方式和砖块、奖励一样。除此之外，还需要一个单球的构造函数。
...
更新与绘制函数需要遍历球容器，对每个球调用 `update_ball` 与 `draw_ball`。
...
做完这些改动后，需要更新碰撞检测。`collisions.resolve_collisions` 的子过程和参数名称也要调整，以体现现在是多个球而不是一个球。
...
游戏状态也需要更新以适配模块名。实际上只需改 “game” 状态，因为它是唯一直接访问 `balls` 的地方。
...
游戏中可能同时有多个球，所以只有当所有球都逃出屏幕后才会扣命。
...
在之前的单球版本里，`ball.reposition` 负责在关卡开始时设置球的位置并重置球。多球情况下，这个功能由 `balls.reset` 取代。
...
另一组必要改动是球对奖励的反应。我们需要把整个 balls 数组传给 `bonuses.bonus_collected`。
...
对于加速/减速奖励，我希望它影响所有球，因此需要遍历 `balls.current_balls`。
...
当粘性奖励生效且多个球粘在平台上时，需要决定释放哪个球。我不想维护“粘住顺序”，就简单地释放 `balls.current_balls` 中第一个带 `stuck_to_platform` 标志的球。
...
当玩家吃到其他奖励时，粘性效果会被解除。这种情况下，要释放所有粘在平台上的球。
...
(\*1)：`balls.launch_all_balls_from_platform` 与 `balls.launch_single_ball_from_platform` 的区别是：前者在 `if` 末尾没有 `break`。
...
最后终于来到本节的核心目标——新增球奖励！需要在 `bonuses.bonus_collected` 里添加对该奖励类型的识别。
...
要真正新增一个球，需要决定它的位置和速度。我复制 `balls.current_balls` 中第一个球的位置、速度和一些参数，把速度旋转 45 度，然后用这些参数构造新球，最后把它插入 `balls.current_balls`。
